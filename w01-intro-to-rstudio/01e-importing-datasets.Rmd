...
- You can see your current RStudio working directory by running the function `getwd()` in the Console.

Once you know the working directory, you can leave it as is or set it to what you want it to be:
...
## Importing datasets with `read.csv()` from base R
...
A more common function when using tidyverse packages and style is `read_csv()`. However, this function does not clean the variable names either. It simply adds `' '` to the names.
...
In order to work with variables from the dataset, we must tell R what dataset the variable is from by using `dataset_name$variable`. The $ operator extracts the variable form the dataset. 
